class Appointment < ActiveRecord::Base
 ...
 #  	#validates_numericality_of :end, :greater_than => :start
	...
	def self.getHoursReport2DArray(users, start_month, start_year, end_month, end_year)  #called in reports controller
		...
		months_to_iter_over.times do
			...
			users.each do |user_name|
				table[this_entry_name][user_name[0]] = {}
				user = User.find_by_name(user_name[0])
				appts = Appointment.where(["start_time >= ? and end_time < ? and user_id=? and participant != ?", this_month, next_month, user, ""]).to_a
				num_hours = 0
				...
				table[this_entry_name][user_name[0]] = num_hours
				user_totals[user_name[0]] += num_hours
			end
		...
		output = []
		table.each do |month, users|
			array = [month]
			users.each do |user_name, hours|
				array << hours
			end
			output << array.to_csv
		end

		output << user_total_arr.to_csv
	...
	def self.getNumPeopleReport2DArray(users, start_month, start_year, end_month, end_year)
		...
		months_to_iter_over.times do
			...
			users.each do |user_name|
				...
				table[this_entry_name][user_name[0]] = unique_person.keys.length
				user_totals[user_name[0]] += unique_person.keys.length
			end
		...
		table.each do |month, users|
			array = [month]
			users.each do |user_name, people|
				array << people
			end
			output << array.to_csv
		end
